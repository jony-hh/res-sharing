...
(
    `id`            bigint           NOT NULL AUTO_INCREMENT COMMENT '雪花漂移id',
    `uid`           bigint UNSIGNED  NOT NULL DEFAULT 0 COMMENT '用户id',
    `identity_type` tinyint UNSIGNED NOT NULL DEFAULT 1 COMMENT '1手机号 2邮箱 3用户名 4qq 5微信 6腾讯微博 7新浪微博',
    `identifier`    varchar(50)      NOT NULL DEFAULT '' COMMENT '身份标识(手机号 邮箱 用户名或第三方应用的唯一标识)',
    `certificate`   varchar(20)      NOT NULL DEFAULT '' COMMENT '密码(站内的保存密码，站外的不保存或保存token)',
...
(
    ...
    `platform`     tinyint      NOT NULL COMMENT '平台类型  0：阿里云   1：腾讯云   2：七牛云   3：华为云',
...
(
    `id`           bigint        NOT NULL AUTO_INCREMENT COMMENT 'id',
    `platform_id`  bigint        NOT NULL DEFAULT '' COMMENT '平台ID',
    `platform`     tinyint       NOT NULL DEFAULT '' COMMENT '平台类型',
    `mobile`       varchar(20)   NOT NULL COMMENT '手机号',
    `params`       varchar(200)  NULL     DEFAULT '' COMMENT '参数',
    `status`       tinyint       NOT NULL DEFAULT '' COMMENT '状态  0：失败   1：成功',
    `error`        varchar(2000) NULL     DEFAULT '' COMMENT '异常信息',
    `created_time` timestamp     NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '发送时间',
    primary key (id)
) ENGINE = InnoDB
